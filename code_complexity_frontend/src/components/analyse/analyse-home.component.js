...
class AnalyseHome extends Component {
  constructor(props) {
    ...
    this.onClickCr = this.onClickCr.bind(this);
  ...
  onClickCtc() {
    axios
      .get(constants.url + "/controlStructure/total/ctc/" + this.state.fileId)
      .then(res => {
        swal(
          "Total complexity of " +
            this.state.file.fileName +
            " due to type of control structures (Ctc) : " +
            res.data
        );
  ...
  onClickCr() {
    axios
      .get(constants.url + "/recursive/analyse/{id}" + this.state.fileId)
      .then(res => {
        swal("Complexity of a program statement due to size " + res.data);
        console.log("dss" + res.data);
      })
      ...
      <>
        <MDBContainer className="w-100 mt-5">
          ...
          <MDBRow>
            ...
            <MDBCol sm="4">
              <MDBBtn onClick={this.onClickCr}>Complexity due to recursion (Cr) </MDBBtn>
            </MDBCol>
          </MDBRow>
          <MDBRow>
            <MDBCol sm="4" />
